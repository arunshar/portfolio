...
and linked by DOI). Each is a code-reference landing page for a numpy reference
...
Run from the repo root:  python3 tools/build_pub_pages.py
It writes projects/<repo>/index.html for each entry, appends them to
projects/manifest.json, and adds a "page" link to the matching publications.js
entry. Idempotent.
"""
import html
import json
...
CANON = "https://arunshar.github.io/portfolio/projects/{repo}/"
...
# pub-id -> repo, package, blurb, algorithm paragraph, measured-result paragraph
PUBS = {
    "intexpl": {
        "repo": "stagd-trajectory-gap", "pkg": "stagd",
    ...
    "sear": {
        "repo": "tgard-rendezvous", "pkg": "tgard",
    ...
    "leap": {
        "repo": "tss-rendezvous-region", "pkg": "tss",
    ...
    "vision-loco": {
        "repo": "webglobe-raster", "pkg": "webglobe",
    ...
    "legmanip": {
        "repo": "stdm-survey-toolkit", "pkg": "stdm",
...
ORDER = ["intexpl", "sear", "leap", "vision-loco", "legmanip"]
...
<section id="reference-implementation">
  ...
  <p>This page accompanies the open-source reference implementation at
    ...
    a faithful numpy reimplementation of the paper's core algorithm on synthetic data, runnable end to end.</p>
...
def main():
    ...
    for pid in ORDER:
        ...
        repo = meta["repo"]
        teaser = ""
        fig = os.path.join(ROOT, "assets", "figures", f"pub-{pid}.png")
        ...
        page = (tmpl
                ...
                .replace("{{CANONICAL}}", CANON.format(repo=repo))
        ...
        outdir = os.path.join(ROOT, "projects", repo)
        os.makedirs(outdir, exist_ok=True)
        open(os.path.join(outdir, "index.html"), "w").write(page)

        # add a "page" link to the publication entry (idempotent)
        links = pub.setdefault("links", [])
        href = f"projects/{repo}/"
        if not any(l.get("href") == href for l in links):
            links.append({"label": "page", "href": href})

        # manifest entry (idempotent)
        if repo not in have:
            manifest.append({
                "slug": repo, "title": pub["title"], "venue": pub.get("venue", ""),
                "blurb": meta["blurb"],
                "teaser": f"assets/figures/pub-{pid}.png",
                "page": f"projects/{repo}/",
                "pdf": pub.get("titleUrl", ""),
                "github": f"https://github.com/arunshar/{repo}",
    ...
    print("generated pages for:", [PUBS[p]["repo"] for p in ORDER])
